...
class LLMTaskProcessor:
    ...
    async def start_async(self, task: LLMTask, skill_class: LLMSkillClass):
        """启动LLM任务处理（异步版本）"""
        self.task = task
        ...
        # 初始化长期持有的帧读取器
        await self._initialize_frame_reader()

        # 创建异步任务
        asyncio.create_task(self._execution_worker_async())

        logger.info(f"LLM任务 {self.task_id} 处理器已启动（异步模式）")
    ...
    def _run_async_task(self, task: LLMTask, skill_class: LLMSkillClass):
        """在新线程中运行异步任务"""
        self.task = task
        self.skill_class = skill_class
        self.running = True
        ...
        try:
            # 初始化帧读取器
            self.loop.run_until_complete(self._initialize_frame_reader())

            # 运行执行工作者
            self.loop.run_until_complete(self._execution_worker_async())
        finally:
            self.loop.close()

    async def _initialize_frame_reader(self):
        """初始化长期持有的帧读取器"""
        try:
            ...
            # 同步调用，因为AdaptiveFrameReader可能是同步的
    ...
    async def _execution_worker_async(self):
        """LLM任务执行工作线程（异步版本）"""
        logger.info(f"LLM任务 {self.task_id} 执行线程已启动（异步模式）")
        ...
        while self.running and not self.stop_event.is_set():
            try:
                # 检查运行时段
                if not await self._is_in_running_period_async():
                    await asyncio.sleep(60)  # 非运行时段，等待1分钟后重新检查
                    continue

                # 执行LLM任务
                start_time = time.time()
                success = await self._execute_llm_task_async()
                ...
                self.stats["errors"] += 1
                await asyncio.sleep(30)  # 异常后等待30秒

        logger.info(f"LLM任务 {self.task_id} 执行线程已停止（异步模式）")

    async def _execute_llm_task_async(self) -> bool:
        """执行单次LLM任务分析（异步版本）"""
        try:
            # 检查帧读取器是否可用
            ...
            # 处理用户提示词模板中的变量替换（使用异步数据库会话）
            if user_prompt:
                db = await get_async_db_session()
                try:
                    # 使用异步查询获取摄像头信息
                    result = await db.execute(
                        select(CameraService.ai_camera_model).filter(
                            CameraService.ai_camera_model.id == self.task.camera_id
                        )
                    )
                    ...
                    if camera_info:
                        camera_data = {
                            "name": camera_info[0].name,
                            "id": camera_info[0].id
                        }
                        user_prompt = user_prompt.replace("{camera_name}", camera_data.get("name", "未知摄像头"))
                        user_prompt = user_prompt.replace("{camera_id}", str(camera_data.get("id", self.task.camera_id)))
                except Exception as e:
            ...
            await self._process_llm_result_async(result_data, frame)
    ...
    async def _process_llm_result_async(self, llm_response: Dict[str, Any], frame: np.ndarray):
        """处理LLM分析结果，根据预警条件生成预警（异步版本）"""
        try:
            # 获取预警条件配置
            ...
            if alert_triggered:
                # 生成预警
                await self._generate_alert_async(llm_response, frame)
    ...
    async def _generate_alert_async(self, analysis_result: Dict[str, Any], frame: np.ndarray):
        """生成预警（异步版本）"""
        try:
            # 获取摄像头信息（使用异步数据库会话）
            db = await get_async_db_session()
            try:
                ...
                if camera_info:
                    camera_data = {
                        "name": camera_info[0].name,
                        "location": camera_info[0].location
                    }
                    camera_name = camera_data.get("name", f"摄像头{self.task.camera_id}")
                    location = camera_data.get("location") or "智能监控区域"
                else:
    ...
    async def _is_in_running_period_async(self) -> bool:
        """检查是否在运行时段内（异步版本）"""
...
class LLMTaskExecutor:
    ...
    def __init__(self):
        ...
        logger.info("LLM任务执行器已初始化（异步模式）")

    async def start_async(self):
        """启动LLM任务执行器（异步版本）"""
        self._loop = asyncio.get_event_loop()
        self.scheduler.start()
        await self.schedule_all_tasks_async()
        logger.info("LLM任务执行器已启动（异步模式）")
    ...
    def start(self):
        ...
        def run_scheduler():
            ...
            self._loop = loop
            self.scheduler.start()
            loop.run_until_complete(self.schedule_all_tasks_async())
            loop.run_forever()

        thread = threading.Thread(target=run_scheduler, daemon=True, name="LLMTaskExecutor")
        thread.start()
        logger.info("LLM任务执行器已启动")

    async def schedule_all_tasks_async(self):
        """调度所有启用的LLM任务（异步版本）"""
        try:
            db = await get_async_db_session()
            try:
                # 获取所有启用的LLM任务
                tasks = await LLMTaskDAO.get_all_enabled_async(db)

                logger.info(f"发现 {len(tasks)} 个启用的LLM任务")

                for task in tasks:
                    try:
                        await self._schedule_task_async(task, db)
                    except Exception as e:
    ...
    async def _schedule_task_async(self, task: LLMTask, db: AsyncSession):
        ...
        try:
            ...
            # 启动任务处理器
            await processor.start_async(task, skill_class)
    ...
    def update_task_schedule(self, task_id: int):
        ...
        async def _update():
            try:
                db = await get_async_db_session()
                try:
                    task = await LLMTaskDAO.get_by_id_async(db, task_id)
        ...
        if self._loop and not self._loop.is_closed():
            ...
            logger.error(f"事件循环不可用，无法更新LLM任务 {task_id} 调度")
