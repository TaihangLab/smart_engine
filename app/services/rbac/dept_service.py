...
class DeptService:
    ...
    async def get_dept_by_id(db: AsyncSession, dept_id: int) -> Optional[SysDept]:
        """根据ID获取部门（异步）"""
        return await RbacDao.dept.get_dept_by_id(db, dept_id)

    @staticmethod
    async def get_all_depts(db: AsyncSession) -> List[SysDept]:
        """获取所有部门（异步）"""
        return await RbacDao.dept.get_all_depts(db)

    @staticmethod
    async def get_dept_by_parent(db: AsyncSession, parent_id: Optional[int]) -> List[SysDept]:
        """获取指定父部门下的所有直接子部门（异步）"""
        return await RbacDao.dept.get_dept_by_parent(db, parent_id)

    @staticmethod
    async def get_dept_subtree(db: AsyncSession, dept_id: int) -> List[SysDept]:
        """获取指定部门及其所有子部门（包括多级子部门）（异步）"""
        return await RbacDao.dept.get_dept_subtree(db, dept_id)

    @staticmethod
    async def update_dept(db: AsyncSession, dept_id: int, update_data: Dict[str, Any]) -> Optional[SysDept]:
        """更新部门信息（异步）"""
        try:
            dept = await RbacDao.dept.update_dept(db, dept_id, update_data)
            if dept:
    ...
    async def delete_dept(db: AsyncSession, dept_id: int) -> bool:
        """删除部门（异步）"""
        try:
            success = await RbacDao.dept.delete_dept(db, dept_id)
            if success:
