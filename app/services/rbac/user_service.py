...
class UserService:
    ...
    @staticmethod
    async def auto_create_user(db: AsyncSession, user_info: Dict[str, Any]) -> SysUser:
        """
        自动创建用户（异步）
        ...
        # 为新用户分配默认角色
        await UserService.assign_default_role(db, user.id, tenant_id)
    ...
    async def delete_user(db: AsyncSession, tenant_id: str, user_name: str) -> bool:
        ...
        success = await RbacDao.user.delete_user(db, user.id)
        if success:
            logger.info(f"删除用户成功: {user.user_name}@{user.tenant_id}")
        return success

    @staticmethod
    async def delete_user_by_id(db: AsyncSession, id: int) -> bool:
        """删除用户（通过用户ID）（异步）"""
        user = await RbacDao.user.get_user_by_id(db, id)
        if not user:
            return False

        success = await RbacDao.user.delete_user(db, id)
        if success:
            logger.info(f"删除用户成功: {user.user_name}@{user.tenant_id}")
        return success
        ...
        for user_id in user_ids:
            ...
            if user:
                ...
                if user.tenant_id in accessible_tenant_ids:
                    success = await RbacDao.user.delete_user(db, user_id)
                    if success:
                        deleted_count += 1
                else:
                    # 用户存在但租户ID不在可访问列表中
