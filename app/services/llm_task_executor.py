...
class LLMTaskExecutor:
    ...
    def __init__(self, use_async: bool = False):
        ...
        Args:
            use_async: 是否使用异步模式（默认False保持向后兼容）
        """
        self.use_async = use_async

        if use_async:
            self.scheduler = AsyncIOScheduler(timezone="Asia/Shanghai")
            self.executor = None  # 异步模式不需要线程池
        else:
        ...
        self._loop = None  # 事件循环（异步模式）

        logger.info(f"LLM任务执行器已初始化（{'异步' if use_async else '同步'}模式）")
    ...
    def start(self):
        ...
        if self.use_async:
            ...
            def run_async_scheduler():
                ...
                loop.run_until_complete(self._schedule_all_tasks_async_wrapper())
                loop.run_forever()

            thread = threading.Thread(target=run_async_scheduler, daemon=True, name="LLMTaskExecutor-Async")
            thread.start()
            logger.info("LLM任务执行器已启动（异步模式）")
        else:
            self.scheduler.start()
            self.schedule_all_tasks()
            logger.info("LLM任务执行器已启动（同步模式）")
    ...
    async def _schedule_all_tasks_async_wrapper(self):
        """异步调度包装器"""
        try:
            ...
            try:
                ...
                logger.info(f"发现 {len(tasks)} 个启用的LLM任务（异步模式）")
