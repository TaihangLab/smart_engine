...
class AlertThreshold():
    """预警阈值枚举"""
    vehicleCount = 1  # 非机动车数量阈值


class BikeDetectorSkill(BaseSkill):
    """非机动车闯入检测技能

    使用YOLO模型检测非机动车闯入情况，基于triton_client全局单例
    """

    # 默认配置
    DEFAULT_CONFIG = {
        "type": "detection",  # 技能类型：检测类
        "name": "bike_detector",  # 技能唯一标识符
        "name_zh": "非机动车闯入检测",  # 技能中文名称
        "version": "1.0",  # 技能版本
        "description": "使用YOLO模型检测非机动车闯入情况",  # 技能描述
        "status": True,  # 技能状态（是否启用）
        "required_models":  ["yolo11_bike"],  # 所需模型
        "params": {
            "classes": ['bicycle','electricVehicle','Motorcycle','tricycle'],
            "conf_thres": 0.5,
            "iou_thres": 0.45,
            "max_det": 300,
            "input_size": [640, 640],
            ...
            # 预警车辆数量阈值配置
            "vehicle_count": AlertThreshold.vehicleCount,
        },
        "alert_definitions": f"当检测到: {AlertThreshold.vehicleCount}辆及以上非机动车闯入时触发, 可在上方齿轮中进行设置。"
    }
    ...
    def _initialize(self) -> None:
        ...
        self.log("info", f"初始化非机动车闯入检测器: model={self.model_name}, classes={self.classes}")
    ...
    def analyze_safety(self, detections):
        ...
        for det in detections:
            ...
            if class_name in ['bicycle','electricVehicle','Motorcycle','tricycle']:
                vehicle_count += 1

        # 计算安全指标
        is_safe = vehicle_count == 0
        ...
        if vehicle_count >= self.vehicle_count:
            ...
            alert_name = "非机动车闯入预警"
            alert_type = "区域安全预警"
            alert_description = f"检测到{vehicle_count}辆非机动车闯入禁行区域，建议立即通知现场安全员进行处理。"
        ...
        self.log(
            "info",
            f"非机动车闯入检测分析：检测到非机动车数量={vehicle_count}，是否触发预警={alert_triggered}"
        )
...
if __name__ == "__main__":
    ...
    detector = BikeDetectorSkill(BikeDetectorSkill.DEFAULT_CONFIG)

    # 测试图像检测
    # test_image = np.zeros((640, 640, 3), dtype=np.uint8)
    # cv2.rectangle(test_image, (100, 100), (400, 400), (0, 0, 255), -1)
    image_path = r"F:/bike.jpg"
