...
class BikeDetectorSkill(BaseSkill):
    ...
    DEFAULT_CONFIG = {
        ...
        "alert_definitions": [
            {
                "level": 1,
                "description": f"当检测到{AlertThreshold.LEVEL_1}辆及以上非机动车闯入时触发。"
            }
        ]
    }
    ...
    def analyze_safety(self, detections):
        ...
        alert_triggered = vehicle_count > 0
        ...
        if alert_triggered:
            # 根据闯入数量确定预警等级，大于等于1辆即为严重预警
            if vehicle_count >= self.level_1_threshold:
                alert_level = 1  # 严重
            ...
            alert_type = "区域安全预警"
            alert_description = (
                f"检测到 {vehicle_count} 辆非机动车闯入禁行区域，"
        ...
        self.log(
            "info",
            f"非机动车闯入检测分析：检测到非机动车数量={vehicle_count}，预警等级={alert_level}"
        )
