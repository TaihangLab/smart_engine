...
class CrowdDetectorSkill(BaseSkill):
    ...
    使用YOLO模型检测人员头部和身体，分析聚集情况，基于triton_client全局单例
    """

    # 默认配置
    DEFAULT_CONFIG = {
        "type": "detection",  # 技能类型：检测类
        "name": "crowd_detector",  # 技能唯一标识符
        "name_zh": "人员聚集检测",  # 技能中文名称
        "version": "1.0",  # 技能版本
        "description": "使用YOLO模型检测人员聚集情况，支持头部和人员检测",  # 技能描述
    ...
    def _calculate_density_level(self, total_people):
        ...
        if total_people >= 20:
            return "极高密度"
        elif 15 <= total_people <= 19:
            return "高密度"
        elif 6 <= total_people <= 14:
            return "中等密度"
        else:
            return "低密度"
    ...
    def _get_detection_point(self, detection: Dict) -> Optional[Tuple[float, float]]:
        """
        获取检测对象的关键点（用于围栏判断）
        对于人员聚集检测，根据类别使用不同的关键点：
        - 头部类别：使用检测框中心点
        - 人员类别：使用检测框底部中心点（脚部位置）
...
if __name__ == "__main__":
    ...
    detector = CrowdDetectorSkill(CrowdDetectorSkill.DEFAULT_CONFIG)

    # 测试图像检测
    # test_image = np.zeros((640, 640, 3), dtype=np.uint8)
    # cv2.rectangle(test_image, (100, 100), (400, 400), (0, 0, 255), -1)
    image_path = "F:/person.jpg"
