...
class GogglesDetectorSkill(BaseSkill):
    """护目镜检测技能
    
    使用YOLO模型检测护目镜，基于triton_client全局单例
    """
    
    # 默认配置
    DEFAULT_CONFIG = {
        "type": "detection",             # 技能类型：检测类
        "name": "goggles_detector",      # 技能唯一标识符
        "name_zh": "护目镜检测",          # 技能中文名称
        "version": "1.0",                # 技能版本
        "description": "使用YOLO模型检测护目镜",  # 技能描述
        "status": True,                  # 技能状态（是否启用）
        "required_models": ["yolo11_goggles"],  # 所需模型
        "params": {
            "classes": ["goggles"],      # 检测类别：护目镜
            "conf_thres": 0.5,
            "iou_thres": 0.45,
            "max_det": 300,
            "input_size": [640, 640],
            "enable_default_sort_tracking": False  # 默认不启用SORT跟踪
        }
    }
    ...
    def _initialize(self) -> None:
        ...
        self.log("info", f"初始化护目镜检测器: model={self.model_name}")
    ...
    def process(self, input_data: Union[np.ndarray, str, Dict[str, Any], Any], fence_config: Dict = None) -> SkillResult:
        ...
        try:
            ...
            result_data = {
                "detections": results,
                "count": len(results),
                "detection_metrics": self.analyze_detections(results)
            }
    ...
    def analyze_detections(self, results):
        ...
        for detection in results:
            ...
            if class_name == "goggles":
                goggles_count += 1
        
        result = {
            "goggles_count": goggles_count,        # 护目镜数量
            "total_detections": len(results),      # 总检测数量
        }
        
        self.log("info", f"护目镜检测分析: 检测到护目镜数量={goggles_count}，总检测数量={len(results)}")
        return result

    def _get_detection_point(self, detection: Dict) -> Optional[Tuple[float, float]]:
        """
        获取检测对象的关键点（用于围栏判断）
        对于护目镜检测，使用检测框的中心点作为关键点
...
if __name__ == "__main__":
    ...
    detector = GogglesDetectorSkill(GogglesDetectorSkill.DEFAULT_CONFIG)
    ...
    # 分析检测状况
    if "detection_metrics" in result.data:
        metrics = result.data["detection_metrics"]
        print(f"检测分析: {metrics}")
