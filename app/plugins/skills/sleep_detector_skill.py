...
class SleepDetectorSkill(BaseSkill):
    ...
    使用YOLO模型检测人员睡岗情况，基于triton_client全局单例
    """

    # 默认配置
    DEFAULT_CONFIG = {
        "type": "detection",  # 技能类型：检测类
        "name": "sleep_detector",  # 技能唯一标识符
        "name_zh": "睡岗检测",  # 技能中文名称
        "version": "1.0",  # 技能版本
        "description": "使用YOLO模型检测岗位人员睡岗情况",  # 技能描述
        "status": True,  # 技能状态（是否启用）
        "required_models": ["yolo11_sleep"],  # 所需模型
        "params": {
            "classes": ["sleep"],
            "conf_thres": 0.5,
            "iou_thres": 0.45,
...
if __name__ == "__main__":
    ...
    detector = SleepDetectorSkill(SleepDetectorSkill.DEFAULT_CONFIG)
