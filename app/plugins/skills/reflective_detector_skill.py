...
class ReflectiveDetectorSkill(BaseSkill):
    ...
    使用YOLO模型检测人员反光衣穿戴佩戴情况，基于triton_client全局单例
    """

    # 默认配置
    DEFAULT_CONFIG = {
        "type": "detection",  # 技能类型：检测类
        "name": "reflective_detector",  # 技能唯一标识符
        "name_zh": "反光衣穿戴检测",  # 技能中文名称
        "version": "1.0",  # 技能版本
        "description": "使用YOLO模型检测相关人员反光衣穿戴佩戴情况",  # 技能描述
        "status": True,  # 技能状态（是否启用）
        "required_models": ["yolo11_reflective"],  # 所需模型
        "params": {
            "classes": ["Reflective-Jacket"],
            "conf_thres": 0.5,
            "iou_thres": 0.45,
            "max_det": 300,
            "input_size": [640, 640],
...
if __name__ == "__main__":
    ...
    detector = ReflectiveDetectorSkill(ReflectiveDetectorSkill.DEFAULT_CONFIG)

    # # 测试图像检测
    # test_image = np.zeros((640, 640, 3), dtype=np.uint8)
    # cv2.rectangle(test_image, (100, 100), (400, 400), (0, 0, 255), -1)
    image_path = "F:/helmet_jacket_00023.jpg"
