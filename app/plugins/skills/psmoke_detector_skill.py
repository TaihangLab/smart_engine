...
class AlertThreshold():
    """预警阈值枚举"""
    smokingPersonCount = 1  # 吸烟人数阈值


class PSmokeDetectorSkill(BaseSkill):
    """吸烟检测技能

    使用YOLO模型检测人员吸烟情况，基于triton_client全局单例
    """

    # 默认配置
    DEFAULT_CONFIG = {
        "type": "detection",  # 技能类型：检测类
        "name": "psmoke_detector",  # 技能唯一标识符
        "name_zh": "吸烟检测",  # 技能中文名称
        "version": "1.0",  # 技能版本
        "description": "使用YOLO模型检测人员吸烟行为",  # 技能描述
        "status": True,  # 技能状态（是否启用）
        "required_models": ["yolo11_smoke"],  # 所需模型
        "params": {
            "classes": ["psmoking"],
            "conf_thres": 0.5,
            "iou_thres": 0.45,
            ...
            # 预警人数阈值配置
            "smoking_person_count": AlertThreshold.smokingPersonCount,
        },
        "alert_definitions": f"当检测到: {AlertThreshold.smokingPersonCount}名及以上人员吸烟时触发, 可在上方齿轮中进行设置。"
    }
    def _initialize(self) -> None:
        ...
        # 预警阈值配置
        self.smoking_person_count = params["smoking_person_count"]
    ...
    def analyze_safety(self, detections):
        ...
        for det in detections:
            ...
            if class_name == 'psmoking':
                smoking_count += 1

        # 计算安全指标
        is_safe = smoking_count == 0
        ...
        if smoking_count >= self.smoking_person_count:
            alert_triggered = True
            alert_name = "人员吸烟预警"
            alert_type = "公共安全预警"
            alert_description = f"检测到{smoking_count}名人员正在吸烟，建议立即通知现场安全员进行处理。"
        ...
        self.log(
            "info",
            f"吸烟检测分析: 检测到吸烟人数={smoking_count}, 是否触发预警={alert_triggered}"
        )
...
if __name__ == "__main__":
    ...
    detector = PSmokeDetectorSkill(PSmokeDetectorSkill.DEFAULT_CONFIG)
