...
class AlertThreshold():
    """预警阈值枚举"""
    unvestedPersonCount = 1  # 未穿着救生衣人数阈值

class LifejacketDetectorSkill(BaseSkill):
    """救生衣穿戴检测技能

    使用YOLO模型检测人员救生衣穿戴佩戴情况，基于triton_client全局单例
    """

    # 默认配置
    DEFAULT_CONFIG = {
        "type": "detection",  # 技能类型：检测类
        "name": "lifejacket_detector",  # 技能唯一标识符
        "name_zh": "救生衣穿戴检测",  # 技能中文名称
        "version": "1.0",  # 技能版本
        "description": "使用YOLO模型检测相关人员救生衣穿戴佩戴情况",  # 技能描述
        "status": True,  # 技能状态（是否启用）
        "required_models": ["yolo11_lifejacket"],  # 所需模型
        "params": {
            ...
            # 预警人数阈值配置
            "un_vested_person_count": AlertThreshold.unvestedPersonCount,
        },
        "alert_definitions": f"当检测到: {AlertThreshold.unvestedPersonCount}名及以上人员未穿着救生衣时触发, 可在上方齿轮中进行设置。"
    }
    ...
    def _initialize(self) -> None:
        ...
        # 预警阈值配置
        self.un_vested_person_count = params["un_vested_person_count"]
    ...
    def analyze_safety(self, detections):
        ...
        Args:
            detections: List[Dict]，模型检测结果（包含 'class_name' 字段）

        Returns:
            Dict: 包含穿戴统计、安全状态、预警信息等内容
        """
        vest_count = 0
        no_vest_count = 0
        ...
        if no_vest_count >= self.un_vested_person_count:
            alert_triggered = True
            alert_name = "未穿戴救生衣"
            alert_type = "作业安全预警"
            alert_description = f"检测到{no_vest_count}名人员未穿戴救生衣（共检测到{total_persons}人），建议立即通知现场安全员进行处理。"

        result = {
            "total_persons": total_persons,
            "compliant_count": vest_count,
            "unsafe_count": no_vest_count,
        ...
        self.log(
            "info",
            f"救生衣穿戴分析: 共检测到 {total_persons} 人，已穿戴={vest_count}，未穿戴={no_vest_count}，是否触发预警={alert_triggered}"
...
if __name__ == "__main__":
    ...
    detector = LifejacketDetectorSkill(LifejacketDetectorSkill.DEFAULT_CONFIG)

    # # 测试图像检测
    # test_image = np.zeros((640, 640, 3), dtype=np.uint8)
    # cv2.rectangle(test_image, (100, 100), (400, 400), (0, 0, 255), -1)
    image_path = "F:/vest.jpg"
