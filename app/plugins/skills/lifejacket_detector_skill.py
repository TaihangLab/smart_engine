...
class LifejacketDetectorSkill(BaseSkill):
    ...
    DEFAULT_CONFIG = {
        ...
        "alert_definitions": [
            {
                "level": 1,
                "description": f"当检测到LEVEL_1: {AlertThreshold.LEVEL_1}名及以上人员未穿着救生衣时触发。"
            },
            {
                "level": 2,
                "description": f"当检测到LEVEL_2: {AlertThreshold.LEVEL_2}名人员未穿着救生衣时触发。"
            },
            {
                "level": 3,
                "description": f"当检测到LEVEL_3: {AlertThreshold.LEVEL_3}名人员未穿着救生衣时触发。"
            },
            {
                "level": 4,
                "description": f"当检测到LEVEL_4: {AlertThreshold.LEVEL_4}名人员未穿着救生衣时触发。"
            }
        ]
    }
    ...
    def analyze_safety(self, detections):
        ...
        if alert_triggered:
            ...
            alert_type = "作业安全预警"
            alert_description = (
                f"检测到 {no_vest_count} 名人员未穿戴救生衣（共检测到 {total_persons} 人），"
        ...
        self.log(
            "info",
            f"救生衣穿戴分析: 共检测到 {total_persons} 人，已穿戴={vest_count}，未穿戴={no_vest_count}，"
            f"预警等级={alert_level}"
        )
