...
class CallPlayDetectorSkill(BaseSkill):
    ...
    DEFAULT_CONFIG = {
        ...
        "params": {
            ...
            # 预警人数阈值配置
            "play_phone_person_count": AlertThreshold.playPhonePersonCount,
        },
        "alert_definitions": f"当检测到: {AlertThreshold.playPhonePersonCount}名及以上人员玩手机时触发, 可在上方齿轮中进行设置。"
    }
    def _initialize(self) -> None:
        ...
        # 预警阈值配置
        self.play_phone_person_count = params["play_phone_person_count"]
    ...
    def analyze_safety(self, detections):
        ...
        if playphone_count >= self.play_phone_person_count:
            alert_triggered = True
            alert_name = "人员玩手机预警"
            alert_type = "工作纪律预警"
            alert_description = f"检测到{playphone_count}名人员正在玩手机，建议立即通知现场安全员进行处理。"
        ...
        self.log(
            "info",
            f"手机使用检测分析: 玩手机={playphone_count}人，总违规={playphone_count}人, 是否触发预警={alert_triggered}"
        )
