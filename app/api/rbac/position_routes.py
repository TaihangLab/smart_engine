...
async def get_position(
    ...
    """根据岗位ID获取岗位详情"""
    try:
        position = await RbacService.get_position_by_id(db, id)
        if not position:
...
async def get_positions(
    ...
    """获取指定租户的岗位列表"""
    try:
        positions = await RbacService.get_positions_by_tenant(db, tenant_id, skip, limit)
        total = await RbacService.get_position_count_by_tenant(db, tenant_id)
...
async def update_position(
    ...
    try:
        ...
        updated_position = await RbacService.update_position(db, id, update_data)
...
async def delete_position(
    ...
    """删除岗位"""
    try:
        success = await RbacService.delete_position(db, id)
        if not success:
            return UnifiedResponse(
                success=False,
                code=404,
