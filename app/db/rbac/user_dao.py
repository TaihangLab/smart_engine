...
class UserDao:
    ...
    @staticmethod
    async def get_users_by_tenant_id(
    ...
    async def update_user(
        ...
        """更新用户信息（异步）"""
        result = await db.execute(select(SysUser).filter(SysUser.id == user_id))
    ...
    @staticmethod
    async def get_user_count_by_tenant_id(db: AsyncSession, tenant_id: str) -> int:
        """根据租户ID获取用户数量（异步）"""
        if tenant_id is None or tenant_id < 0:
            raise ValueError("tenant_id 必须是有效的正整数")
        result = await db.execute(
    ...
    async def get_user_count_advanced_search(
        ...
        if position_code:
            ...
            user_stmt = user_stmt.filter(
                SysUser.position_id.in_(select(position_subq.c.id))
            )
    ...
    @staticmethod
    async def delete_user_by_username_and_tenant_id(
        db: AsyncSession, tenant_id: str, user_name: str
    ) -> bool:
        """根据用户名和租户ID删除用户（异步）"""
        result = await db.execute(
            select(SysUser).filter(
