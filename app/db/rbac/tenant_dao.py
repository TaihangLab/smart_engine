...
class TenantDao:
    ...
    @staticmethod
    async def create_tenant(db: AsyncSession, tenant_data: dict) -> SysTenant:
        """创建租户（异步）"""
    ...
    @staticmethod
    async def get_tenants_by_id_contains(db: AsyncSession, id_part: str, skip: int = 0, limit: int = 100) -> List[SysTenant]:
        """根据租户ID模糊查询租户列表（异步）"""
        result = await db.execute(
            select(SysTenant).filter(
    ...
    @staticmethod
    async def get_tenant_count_by_id_contains(db: AsyncSession, id_part: str) -> int:
        """根据租户ID模糊查询租户数量（异步）"""
        result = await db.execute(
            select(func.count()).select_from(
                select(SysTenant).filter(
    ...
    @staticmethod
    async def get_all_tenants_for_export(db: AsyncSession) -> List[SysTenant]:
        """获取所有租户数据用于导出（异步）"""
        result = await db.execute(
            select(SysTenant).filter(
                SysTenant.is_deleted == False
    ...
    @staticmethod
    async def get_filtered_tenants_for_export(db: AsyncSession,
        ...
        if company_name:
            stmt = stmt.filter(SysTenant.company_name.contains(company_name))
        if status is not None:
            stmt = stmt.filter(SysTenant.status == status)

        result = await db.execute(stmt)
        return list(result.scalars().all())

    @staticmethod
    async def get_tenants_by_ids(db: AsyncSession, tenant_ids: list) -> List[SysTenant]:
        """根据ID列表获取租户（异步）"""
        result = await db.execute(
            select(SysTenant).filter(
    ...
    @staticmethod
    async def batch_delete_tenants_by_ids(db: AsyncSession, tenant_ids: list) -> int:
        """按ID批量删除租户（异步）"""
        result = await db.execute(
            select(SysTenant).filter(
                SysTenant.id.in_(tenant_ids),
