...
class RoleDao:
    ...
    @staticmethod
    async def get_role_by_code_and_tenant_id(
    ...
    @staticmethod
    async def get_role_by_code(db: AsyncSession, role_code: str, tenant_id: str):
        """根据角色编码获取角色（异步）"""
    ...
    @staticmethod
    async def get_roles_by_tenant_id(
    ...
    @staticmethod
    async def update_role_by_code(
        db: AsyncSession, role_code: str, tenant_id: str, update_data: dict
    ):
            ...
            for key, value in update_data.items():
                if hasattr(role, key):
                    ...
                    if key == "role_code":
                        ...
                        if existing:
                            raise ValueError(
                                f"角色编码 {value} 在租户 {tenant_id} 中已存在"
                            )
    ...
    @staticmethod
    async def delete_role_by_code(db: AsyncSession, role_code: str, tenant_id: str):
        """根据角色编码和租户编码删除角色（异步）"""
        result = await db.execute(
            select(SysRole).filter(
                SysRole.role_code == role_code, SysRole.tenant_id == tenant_id
    ...
    async def update_role(db: AsyncSession, role_id: int, update_data: dict):
        """更新角色信息（异步）"""
        result = await db.execute(select(SysRole).filter(SysRole.id == role_id))
        role = result.scalars().first()
        if role:
            for key, value in update_data.items():
                if hasattr(role, key):
    ...
    @staticmethod
    async def get_role_count_by_tenant_id(db: AsyncSession, tenant_id: str):
        """根据租户ID获取角色数量（异步）"""
        result = await db.execute(
    ...
    @staticmethod
    async def get_role_count_by_tenant(db: AsyncSession, tenant_id: str):
        """获取租户下的角色数量（异步）"""
        # 由于tenant_id字段已被替换为tenant_id，需要先将tenant_id转换为tenant_id
        try:
            tenant_id = int(tenant_id)
    ...
    @staticmethod
    async def get_roles_advanced_search_by_tenant_id(
        db: AsyncSession,
        tenant_id: str,
        role_name: str = None,
        role_code: str = None,
    ...
    @staticmethod
    async def get_roles_advanced_search(
        db: AsyncSession,
        tenant_id: str,
        role_name: str = None,
        role_code: str = None,
    ...
    @staticmethod
    async def get_role_count_advanced_search_by_tenant_id(
        db: AsyncSession,
        tenant_id: str,
        role_name: str = None,
        role_code: str = None,
    ...
    @staticmethod
    async def get_role_count_advanced_search(
        db: AsyncSession,
        tenant_id: str,
        role_name: str = None,
        role_code: str = None,
